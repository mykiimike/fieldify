...
class fieldifyIteratorBuffer {
    ...
    branch(schema, input, end, line) {
        ...
        // push the current access state
        // and apply the new one by merging
        this._accessStack.push(this.access)
        this.access = Object.assign(this.access, tmpAccess)

        //console.log("Level UP "+this.level, this.access);
        ...
        utils.eachItem(nestedObject, (index, ptr, next, oend) => {
            if (oend === true) {
                ...
                // at each level less we restore the 
                // access state of the lower step
                this.access = this._accessStack.pop();
                ...
                if (!Array.isArray(current.input)) {
                    ...
                    utils.eachItem(current.input, (index, ptr, next2, oend) => {
                        ...
                        // here we can get and compare values
                            ...
                            current.access = Object.assign(this.access, arraySchema);
                            current.result = {}
            ...
            // here we can get and compare values
                ...
                // creation of a shortcut on the access fields
                current.access = Object.assign(this.access, current.object);
