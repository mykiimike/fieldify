...
const ftEmailVerify = async (data) => {
    ...
    const acceptPunnycode = configValue(ftEmailConfiguration, params, "acceptPunnycode");
    ...
    if (dRet.punnycode === true && acceptPunnycode !== true) {
        data.error = 'Punnycode is forbidden in the domain';
        return;
    }
 ...
 * @property {boolean} acceptPunnycode - Accept punnycode in the domain name.
 */
...
const ftEmailConfiguration = {
    ...
    acceptPunnycode: {
        $doc: 'Accept punnycode in the domain name',
        $help: 'Accept punnycode in the domain name',
        $required: false,
        $type: 'Boolean',
        $default: false
    }
