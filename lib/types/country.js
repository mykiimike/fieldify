...
const ftCountryExtractor = async (data) => {
    ...
    if (Array.isArray(allowedCountries)) {
        ...
        for (var allowed of allowedCountries) {
            const allowedCountry = countriesDetection[allowed]
            if (allowedCountry && ret.country.alpha3 === allowedCountry.alpha3)
                found = true
        }
        if (found === false) {
...
const ftCountryDecode = async (data) => {
    data.result = await ftCountryExtractor(data)
};

const ftCountryVerify = async (data) => {
    await ftCountryExtractor(data)
    data.result = data.value
};
