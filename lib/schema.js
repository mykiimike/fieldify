...
class fieldifySchema {
  ...
  compile (schema) {
    ...
    var extract = assign(this.tree, localAssigner)

    // assign sub fields
    extract = assign(extract, (user, dst, object, source) => {
    ...
    this.handler = compile(extract, this.resolver.bind(this))
  }
  ...
  export (pv) {
    const localAssigner = (user, dst, object, source) => {
      if (object.$hidden === true) return (true)
      for (var a in object) {
