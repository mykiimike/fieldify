...
describe('Testing schema.verify()', function () {
    ...
    it('verification error should respect dot in field description', function (done) {
        const sc = {
            super: {
                nested1: {
                    $type: "String",
        ...
        const input = { super: { nested1: "errére", nested2: "errére" } }
        const hdl = new schema('test', sc)
        hdl.compile(sc)
        hdl.verify(input, (fieldified) => {
            if (!fieldified.fields["super.nested1"]) return (done('Invalid error field'))
            done()
        })
    })
})
